import { FaBars, FaEarthAsia, FaUser, FaSistrix, FaHeart } from 'react-icons/fa6'
    ...
    <div className='font-roboto overflow-y-scroll h-screen' onScroll={handleScroll}>
      <div className={clsx('sticky top-0 bg-white z-10', scrollStarted && 'shadow')}>
        <div className='flex flex-col'>
          <div className='w-full flex items-center justify-between h-[80px] px-10'>
            ...
            <div className='flex items-center justify-center px-6 font-semibold text-gray-400'>
              {navigationMenu.map(({ label, path }, idx) => {
                ...
                return (
                  <button
                    ...
                    className={clsx(
                      'select-none px-4 py-3 rounded-full hover:bg-black/10',
          ...
          <div className='w-full flex items-center justify-center h-[66px] mb-4'>
            <div className='border border-gray-300 rounded-full shadow-lg flex relative'>
              <button className='pl-9 pr-6 py-3 hover:bg-black/10 rounded-full cursor-pointer text-start'>
                <p className='text-sm text-gray-500 font-semibold select-none'>Tags</p>

                <input placeholder='Search by Tags' className='text-sm outline-none bg-transparent' />
              </button>

              <div className='flex items-center'>
                <hr className='border-l border-gray-300 w-[1px] h-9' />
              </div>

              <button className='pl-9 pr-6 py-3 hover:bg-black/10 rounded-full cursor-pointer text-start min-w-48'>
                <p className='text-sm text-gray-500 font-semibold select-none'>Date</p>

                <p className='text-sm text-gray-400'>Select Date</p>
              </button>

              <button className='absolute top-0 right-0 p-4 m-2 bg-[#61dafb] rounded-full'>
                <FaSistrix size='20' className='text-white' />
              </button>
            </div>
          </div>
        </div>
